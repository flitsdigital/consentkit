...
export type Parts = {
  ...
  preview: ReactNode;
  version: string;
  // PROTOTYPE (branch prototype/mobbin): extra's uit de Mobbin-review; undefined = huidige UI
  proto?: { extractStatus: ReactNode; exportMenu: ReactNode; emptyState: ReactNode; artboard: string; undo: () => void };
};
...
const Icon = {
  ...
  Undo: () => svg(<><path d="M3 7h7a3 3 0 0 1 0 6H6" /><path d="M6 4L3 7l3 3" /></>),
...
const SECTIONS: Record<Tab, Section[]> = {
  ...
  stijl: [
    { id: "site", label: "Site", icon: <Icon.Globe />, render: (p) => <div className="space-y-3 px-3 pb-4"><div>{p.urlForm}</div>{p.proto ? p.proto.extractStatus : p.errorLine}{p.foundPanel}</div> },
...
export function Studio(p: Parts) {
  const [open, setOpen] = useState<string | null>(p.proto?.emptyState ? null : "stijl.site"); // PROTOTYPE: lege staat = geen paneel
  const hadEmpty = useRef(!!p.proto?.emptyState);
  useEffect(() => { if (hadEmpty.current && !p.proto?.emptyState) { hadEmpty.current = false; setOpen("stijl.site"); } }, [p.proto?.emptyState]);
  ...
  useEffect(() => {
    ...
    const onOpen = (e: Event) => { const t = (e as CustomEvent<Tab>).detail; setTab(t); setOpen(`${t}.${SECTIONS[t][0].id}`); }; // PROTOTYPE: vanuit het export-menu
    ...
    <div className="grid h-full grid-rows-[48px_1fr_48px]">
      <header className="grid grid-cols-[1fr_auto_1fr] items-center border-b border-line px-4">
        ...
        <div className="flex items-center justify-end gap-2">
          {p.proto && <button type="button" className="btn btn-ghost h-8 w-8 px-0" title="Ongedaan maken (⌘Z)" aria-label="Ongedaan maken" onClick={p.proto.undo}><Icon.Undo /></button>}
          {p.shareBtn}
          {p.proto ? (
            <span className="relative">
              <button type="button" className="btn btn-primary ps-3 pe-2.5" popoverTarget="export-menu">Exporteren ▾</button>
              {p.proto.exportMenu}
            </span>
          ) : (
            <button type="button" className="btn btn-primary ps-3 pe-2.5" onClick={() => pickTab("installatie")}>Exporteren <Arrow /></button>
          )}
        </div>
      </header>
        ...
        <main className="canvas absolute inset-0 flex items-center justify-center overflow-auto p-8 pl-28">
          {p.proto?.emptyState ?? (
            <div className="relative w-full max-w-5xl">
              {p.proto && <div className="mb-2 text-center font-mono text-[11px] text-muted">{p.proto.artboard}</div>}
              {p.preview}
            </div>
          )}
        </main>
        ...
        <nav className="absolute top-4 left-4 flex w-[68px] flex-col items-stretch gap-0.5 rounded-xl bg-panel p-1.5" style={{ boxShadow: "var(--shadow-pop)" }} aria-label="Secties">
          {TAB_IDS.map((t, gi) => (
            <div key={t} className="flex flex-col gap-0.5">
              {gi > 0 && (p.proto ? <span className="h-1" /> : <span className="mx-2 my-1 h-px bg-line" />)}
              {SECTIONS[t].map((s) => (
...
/* PROTOTYPE switcher (alleen dev) — A = huidig, B = Mobbin-verbeteringen */
const VARIANTS: [string, string][] = [["A", "Huidig"], ["B", "Mobbin: lege staat, statussen, kleurkaarten, checklist, export-menu, undo, artboard-label"]];
export function PrototypeSwitcher({ current }: { current: string }) {
  const router = useRouter();
  const i = Math.max(0, VARIANTS.findIndex(([k]) => k === current));
  const go = (d: number) => { const q = new URLSearchParams(location.search); q.set("variant", VARIANTS[(i + d + VARIANTS.length) % VARIANTS.length][0]); router.replace(`?${q}`); };
  if (process.env.NODE_ENV === "production") return null;
  return (
    <div className="fixed bottom-14 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-full bg-accent px-2 py-1 text-[12px] font-medium text-white shadow-lg">
